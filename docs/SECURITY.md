# 🔒 安全分析报告

本文档分析了项目的安全风险，特别是关于 API Key 泄露的问题。

## ✅ 已实施的安全措施

### 1. 环境变量管理 ✅
- ✅ `.env` 文件已正确添加到 `.gitignore`，不会被提交到 Git
- ✅ 所有敏感配置都通过环境变量加载，没有硬编码
- ✅ `.env.example` 只包含占位符，不包含真实密钥
- ✅ 使用 `python-dotenv` 安全加载环境变量

### 2. GitHub Actions 安全 ✅
- ✅ 使用 GitHub Secrets 存储敏感信息
- ✅ 工作流中通过 `${{ secrets.XXX }}` 引用，不会在日志中暴露
- ✅ 日志中只显示 API Key 前8位用于调试

### 3. 日志安全 ✅
- ✅ 代码中只显示 API Key 前8位（如 `key[:8]...`）
- ✅ 敏感信息不会完整记录到日志文件
- ✅ 使用占位符显示配置状态

### 4. 代码安全 ✅
- ✅ 没有硬编码的真实 API Key
- ✅ 没有硬编码的密码或 Token
- ✅ 配置加载使用安全的单例模式

## ⚠️ 发现的安全问题

### 🔴 严重问题

#### 1. ~~test_env.py / main.py 中硬编码代理配置~~ ✅ 已修复

**原位置：** `test_env.py`、`main.py` 中曾硬编码 `http://127.0.0.1:10809`

**修复状态：** 已统一改为从 `.env` 的 `HTTP_PROXY`/`HTTPS_PROXY` 读取，无配置则直连

**原风险：**
- 暴露本地代理配置
- 如果提交到 Git，会泄露本地网络配置
- 可能被恶意利用

**已采纳的修复：** 从 `.env` 或环境变量读取 `HTTP_PROXY`/`HTTPS_PROXY`，无配置则直连。

### 🟡 中等问题

#### 2. WebUI 缺少认证和授权机制

**位置：** `web/` 目录下的所有 Web 服务

**风险：**
- WebUI 完全开放，任何人都可以访问和修改配置
- 可以修改 `.env` 文件中的 `STOCK_LIST`
- 可以触发股票分析任务，消耗 API 配额
- 如果部署在公网，存在严重安全风险

**修复建议：**
1. **添加基本认证（推荐）：**
   ```python
   # 在 web/handlers.py 中添加认证装饰器
   import base64
   from functools import wraps
   
   def require_auth(f):
       @wraps(f)
       def wrapper(*args, **kwargs):
           auth_header = request.headers.get('Authorization', '')
           if not auth_header.startswith('Basic '):
               return Response('Unauthorized', 401)
           # 验证用户名密码
           return f(*args, **kwargs)
       return wrapper
   ```

2. **使用环境变量配置认证：**
   ```bash
   # .env
   WEBUI_USERNAME=admin
   WEBUI_PASSWORD=your_secure_password
   ```

3. **或使用 Token 认证：**
   ```bash
   # .env
   WEBUI_TOKEN=your_random_token_here
   ```

4. **限制访问来源（如果可能）：**
   - 使用反向代理（Nginx）添加 IP 白名单
   - 或使用防火墙限制访问

**修复优先级：** 🟡 中（如果部署在公网则为 🔴 高）

#### 3. .env 文件写入缺少验证

**位置：** `web/services.py` 的 `ConfigService.set_stock_list()` 方法

**风险：**
- 虽然只修改 `STOCK_LIST`，但写入操作没有充分验证
- 如果正则表达式被绕过，可能修改其他配置
- 没有路径验证，理论上可能存在路径遍历风险

**修复建议：**
```python
def set_stock_list(self, stock_list: str) -> str:
    # 验证股票代码格式
    normalized = self._normalize_stock_list(stock_list)
    
    # 验证每个股票代码格式
    for code in normalized.split(','):
        if not self._is_valid_stock_code(code.strip()):
            raise ValueError(f"无效的股票代码: {code}")
    
    # 验证文件路径（防止路径遍历）
    if not os.path.abspath(self.env_path).startswith(os.path.abspath('.')):
        raise ValueError("非法的文件路径")
    
    # 然后才写入
    ...
```

**修复优先级：** 🟡 中

#### 4. 日志中可能泄露部分敏感信息

**位置：** 多个文件

虽然只显示前8位，但在某些情况下可能仍然有风险：

- `test_env.py:69, 73` - 显示 Token/Key 前8位
- `src/search_service.py:120` - 显示 API Key 前8位
- `.github/scripts/ai_review.py:87, 121` - 显示 API Key 前8位

**风险：**
- 虽然只显示前8位，但在某些场景下可能被利用
- 如果多个 Key 前8位相同，可能被关联

**修复建议：**
- 考虑使用哈希值替代直接显示
- 或完全移除调试输出中的 Key 信息
- 使用环境变量控制是否显示调试信息

**修复优先级：** 🟡 中

#### 3. .env.example 中的示例值

**位置：** `.env.example`

虽然只是示例，但包含了一些可能被误解为真实配置的值：

```bash
DINGTALK_APP_KEY=xxxx
DINGTALK_APP_SECRET=xxxx
FEISHU_APP_ID=xxxx
FEISHU_APP_SECRET=xxxx
```

**风险：**
- 用户可能误以为这些是有效的占位符
- 可能被用于测试，导致配置错误

**修复建议：**
- 使用更明确的占位符，如 `your_dingtalk_app_key_here`
- 添加注释说明这是示例值

**修复优先级：** 🟡 低

### 🟢 低风险问题

#### 5. Web 服务缺少安全响应头

**位置：** `web/server.py`, `web/handlers.py`

**风险：**
- 没有设置安全响应头（CORS、X-Frame-Options 等）
- 可能被用于 XSS 攻击或点击劫持

**修复建议：**
```python
# 在 Response 类中添加安全头
def send(self, handler: 'BaseHTTPRequestHandler') -> None:
    handler.send_response(self.status)
    handler.send_header("Content-Type", self.content_type)
    handler.send_header("Content-Length", str(len(self.body)))
    # 添加安全响应头
    handler.send_header("X-Content-Type-Options", "nosniff")
    handler.send_header("X-Frame-Options", "DENY")
    handler.send_header("X-XSS-Protection", "1; mode=block")
    # 如果不需要跨域，可以设置
    # handler.send_header("Access-Control-Allow-Origin", "null")
    handler.end_headers()
    handler.wfile.write(self.body)
```

**修复优先级：** 🟢 低

#### 6. Webhook URL 可能包含敏感信息

**位置：** 配置文件、日志

Webhook URL 通常包含 Token，虽然不会直接暴露，但需要注意：

- 如果 Webhook URL 被记录到日志，可能泄露
- 如果 Webhook URL 被打印到控制台，可能被截屏泄露

**修复建议：**
- 确保 Webhook URL 不会完整记录到日志
- 在显示时进行脱敏处理

**修复优先级：** 🟢 低

## 📋 安全检查清单

### 代码检查
- [x] 没有硬编码的 API Key
- [x] 没有硬编码的密码
- [x] 没有硬编码的 Token
- [x] `.env` 文件已添加到 `.gitignore`
- [x] `.env.example` 只包含占位符
- [x] ✅ `main.py`、`test_env.py` 中的硬编码代理已移除，统一从 `.env` 读取

### Web 服务检查
- [ ] ⚠️ WebUI 缺少认证机制（如果部署在公网，需要修复）
- [ ] ⚠️ `.env` 文件写入缺少充分验证
- [ ] ⚠️ 缺少安全响应头（CORS、X-Frame-Options 等）
- [x] 输入验证（股票代码格式验证）✅
- [x] 使用参数化查询（SQLAlchemy ORM）✅

### 日志检查
- [x] 日志中只显示 API Key 前8位
- [x] 敏感信息不会完整记录
- [x] 使用占位符显示配置状态

### GitHub Actions 检查
- [x] 使用 GitHub Secrets 存储敏感信息
- [x] 工作流中正确引用 Secrets
- [x] 日志中不会暴露完整密钥

### Docker/Podman 检查
- [x] 环境变量通过 `.env` 文件加载
- [x] 容器中不会硬编码敏感信息
- [x] 卷挂载不会暴露敏感文件

## 🛡️ 安全最佳实践建议

### 1. 立即修复的问题

#### ~~修复硬编码代理~~ ✅ 已完成

所有入口（`main.py`、`test_env.py`、`webui.py`）均通过 `src.config` 加载 `.env`，代理由 `HTTP_PROXY`/`HTTPS_PROXY` 配置，无配置则直连。配置后会自动设置 `NO_PROXY` 排除国内数据源。

### 2. 增强日志安全

考虑使用哈希值替代直接显示：

```python
import hashlib

def mask_api_key(key: str) -> str:
    """生成 API Key 的哈希标识"""
    if not key:
        return "未配置"
    hash_obj = hashlib.sha256(key.encode())
    return f"key_{hash_obj.hexdigest()[:8]}"
```

### 3. 环境变量验证

添加环境变量验证，确保敏感配置不会意外泄露：

```python
def validate_env_security():
    """验证环境变量安全性"""
    sensitive_vars = [
        'GEMINI_API_KEY',
        'OPENAI_API_KEY',
        'EMAIL_PASSWORD',
        'TELEGRAM_BOT_TOKEN',
    ]
    
    for var in sensitive_vars:
        value = os.getenv(var)
        if value and len(value) < 10:
            logger.warning(f"{var} 长度异常，可能配置错误")
```

### 4. 使用密钥管理服务

对于生产环境，建议使用密钥管理服务：

- **本地开发：** 使用 `.env` 文件（已实施）
- **Docker/Podman：** 使用环境变量或密钥卷
- **GitHub Actions：** 使用 GitHub Secrets（已实施）
- **生产环境：** 考虑使用 HashiCorp Vault、AWS Secrets Manager 等

### 5. 定期安全审计

建议定期执行以下检查：

1. **检查 Git 历史：** 确保没有敏感信息被提交
   ```bash
   git log --all --full-history --source -- "*env*" "*key*" "*token*"
   ```

2. **检查代码库：** 搜索可能的硬编码密钥
   ```bash
   grep -r "sk-[a-zA-Z0-9]" --exclude-dir=.git
   grep -r "AIza[a-zA-Z0-9]" --exclude-dir=.git
   ```

3. **检查依赖：** 确保依赖包没有已知安全漏洞
   ```bash
   pip install safety
   safety check
   ```

## 🔍 如何检查是否泄露

### 1. 检查 Git 历史

```bash
# 检查是否有敏感文件被提交
git log --all --full-history --source -- ".env"

# 检查是否有 API Key 被提交
git log -p --all -S "GEMINI_API_KEY" --source
git log -p --all -S "OPENAI_API_KEY" --source
```

### 2. 检查当前代码

```bash
# 搜索可能的硬编码密钥
grep -r "sk-[a-zA-Z0-9]\{32,\}" --exclude-dir=.git
grep -r "AIza[a-zA-Z0-9]\{32,\}" --exclude-dir=.git
grep -r "ghp_[a-zA-Z0-9]\{32,\}" --exclude-dir=.git
```

### 3. 检查 GitHub Secrets

确保所有敏感信息都存储在 GitHub Secrets 中，而不是代码中。

## 📝 总结

### 总体安全评级：🟡 中等（本地使用） / 🔴 高风险（公网部署）

**本地使用场景：** 安全措施基本到位
- ✅ 没有硬编码的真实 API Key
- ✅ 正确使用环境变量和 Secrets
- ✅ 日志安全处理得当
- ✅ `.gitignore` 配置正确
- ✅ 输入验证基本完善
- ✅ SQL 注入防护到位

**公网部署场景：** 存在严重安全风险
- ❌ WebUI 完全开放，无认证
- ❌ 任何人都可以修改配置和触发分析
- ❌ 缺少安全响应头

### 需要立即修复的问题：

1. **🔴 高优先级（已修复）：** `test_env.py` 中的硬编码代理配置 ✅
2. **🔴 高优先级（公网部署）：** WebUI 缺少认证机制
3. **🟡 中优先级：** `.env` 文件写入缺少充分验证
4. **🟡 中优先级：** 考虑增强日志脱敏
5. **🟢 低优先级：** 添加安全响应头
6. **🟢 低优先级：** 改进 `.env.example` 的占位符

### 建议：

1. ✅ **已完成：** 修复 `main.py`、`test_env.py` 中的硬编码代理，统一从 `.env` 读取
2. **如果部署在公网：** 必须添加 WebUI 认证机制
3. **增强 `.env` 文件写入验证：** 添加更严格的输入验证
4. **定期执行安全审计：** 检查依赖包漏洞
5. **考虑使用密钥管理服务：** 生产环境推荐
6. **添加安全测试到 CI/CD 流程：** 自动化安全检查
7. **使用反向代理：** Nginx 等可以提供额外的安全层

---

**最后更新：** 2026-01-26

**审核人：** AI Security Analysis
